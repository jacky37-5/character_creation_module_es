...
def start_training(char_name: str, char_class: str) -> str:
    ...
    if char_class == 'magician':
        print(f'{char_name}, eres un Mago,'
              'dominando magistralmente los elementos.')
    ...
    print('Introduce uno de estos comandos: atacar, para atacar un enemigo; '
          'defender, para bloquear un ataque enemigo,'
...
def choice_char_class() -> str:
    ...
    while approve_choice != 'y':
        char_class = input('Introduce la clase de tu personajes: Guerrero,'
                           'guerrero; Mago, mago; Sanador, sanador: ')
        if char_class == 'warrior':
            print('Guerrero: un feroz luchador cuerpo a cuerpo.'
                  'Fuerte, resiliente y valiente.')
        if char_class == 'magician':
            print('Mago: un brillante luchador de largo'
                  'alcance. Maestro de poderes mágicos.')
        if char_class == 'healer':
            print('Sanador: un poderoso chamán. Extrae fuerza'
                  'de la naturaleza, la fe y los espíritus.')
        approve_choice = input('Presiona (Y) para confirmar o cualquier otro'
                               'botón para seleccionar'
                               'cualquier otra clase').lower()
    return char_class
